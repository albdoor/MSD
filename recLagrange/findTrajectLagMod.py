    ...
    def compute_M(self, theta):
        ...
        for i in range(n):
            for j in range(n):
                ...
                term1 = 0
                for k in range(a, n):
                    term1 += m[k] * self.compute_lambda(i, k) * self.compute_lambda(j, k)

                I = inertia(m[i], l[i])

                M_ij = term1 * np.cos(theta[i] - theta[j]) + I * kron_delta(i, j)
                M[i, j] = M_ij
    ...
    def compute_C(self, theta, theta_dot):
        ...
        for i in range(n):
            for j in range(n):
                ...
                C_ij = term1 * l[j] * np.sin(theta[i] - theta[j]) * theta_dot[j]
